<template>
  <div class="pay">
    ...
    <div class="pay-list">
      <div class="list">
        <div v-for="item in goodsList" :key="item.goods_id" class="goods-item">
            <div class="left">
      ...
      <div class="pay-detail">
        ...
        <div class="pay-cell">
          <span>配送费用：</span>
          <span v-if="!order.existAddress">请先选择配送地址</span>
          <span v-else class="red">+￥{{ order.expressPrice }}</span>
        </div>
      </div>
    ...
    <div class="footer-fixed">
      ...
      <div @click="commitOrder" class="tipsbtn">提交订单</div>
    </div>
  </div>
</template>
...
import { checkOrder, commitOrder } from '@/apis/order'
export default {
  name: 'PayIndex',
  data () {
    return {
  ...
  computed: {
    // 这里由于不是主线任务,默认选择列表中第一个作为地址
    selAddress () {
      return this.addressList[0] || {}
    },
    region () {
      return `${this.selAddress.region.province} ${this.selAddress.region.city}  ${this.selAddress.region.region} ${this.selAddress.detail}`
    },
    mode () {
